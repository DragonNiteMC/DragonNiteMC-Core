...
import com.dragonite.mc.dnmc.core.config.serializer.NMSUtils;
...
public final class MessageFactory implements AbstractMessageBuilder {
    ...
    public AbstractMessageBuilder showItem(ItemStack item) {
        ...
        if (json == null) {
            DragoniteMC.plugin.getLogger().warning("Item show for " + item.getType() + "is invalid");
            return this;
        }
    ...
    public AbstractMessageBuilder showEntity(Entity entity) {
        ...
        if (json == null) {
            DragoniteMC.plugin.getLogger().warning("Item show for " + entity.getType() + "is invalid");
            return this;
        }
    ...
    public BaseComponent[] build() {
        if (this.runner != null) {
            ...
            if (timeoutSeconds > 0) {
                DragoniteMC.getChatRunnerHandler().registerTimeout(id, runner, timeoutSeconds);
            } else if (timeoutClicks > 0) {
                DragoniteMC.getChatRunnerHandler().registerClicks(id, runner, timeoutClicks);
            }
        }
