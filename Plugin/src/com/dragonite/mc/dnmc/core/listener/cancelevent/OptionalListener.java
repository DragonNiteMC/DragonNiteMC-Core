package com.dragonite.mc.dnmc.core.listener.cancelevent;

import com.destroystokyo.paper.event.entity.EntityKnockbackByEntityEvent;
import com.dragonite.mc.dnmc.core.config.implement.DNMCoreConfig;
import com.dragonite.mc.dnmc.core.config.implement.yaml.CancelEventConfig;
...
public class OptionalListener implements Listener {
    ...
    public OptionalListener() {
        ...
        if (section.isEmpty()) {
            DragoniteMC.plugin.getLogger().info("Cancel Event is empty, skipped.");
            return;
        }
        ...
        for (String eventName : section.keySet()) {
            ...
            if (eventClass != null){
                Bukkit.getServer().getPluginManager().registerEvent(eventClass, this, EventPriority.NORMAL, (listener, event) -> this.onAllEvent(event), DragoniteMC.plugin);
            }else{
                DragoniteMC.plugin.getLogger().warning(eventName+" is not exist, ignored cancel handle.");
            }
        }

        cancelEventManager = (CancelEventManager) DragoniteMC.getAPI().getEventCancelManager();
