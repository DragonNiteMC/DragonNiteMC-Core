...
import com.dragonnite.mc.dnmc.core.builders.AdvMessageBuilder;
import com.dragonnite.mc.dnmc.core.builders.MessageBuilder;
...
class WorldCreationBuilder {
    ...
    private void confirmCreation(Player sender) {
        ...
        new MessageBuilder("&e[ 確定創建 ]").hoverText("&d點擊確認").runClicks(1, player -> {
            try {
                DragonNiteMC.getDnmcWorldManager().createWorld(world, worldType, environment, generateStructure).whenComplete((w, ex) -> {
                    if (ex != null) ex.printStackTrace();
                    var result = w != null;
                    sender.sendMessage(DragonNiteMC.getDnmCoreConfig().getPrefix() + "§e世界 " + world + " 創建 " + (result ? "成功" : "失敗") + "。");
                });
                sender.sendMessage(DragonNiteMC.getDnmCoreConfig().getPrefix() + "§e正在創建世界....");
