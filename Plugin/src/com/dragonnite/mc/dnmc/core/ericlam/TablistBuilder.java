package com.dragonnite.mc.dnmc.core.ericlam;
...
import com.dragonnite.mc.dnmc.core.managers.TabListManager;
...
public class TablistBuilder implements TabListManager {
    ...
    public void setHeader(String header, Player player) {
        ...
        packet.getChatComponents().write(1, WrappedChatComponent.fromText("§eplay.dragonnite.com"));

        try {
            ProtocolLibrary.getProtocolManager().sendServerPacket(player, packet);
        } catch (InvocationTargetException | NullPointerException e) {
            DragonNiteMC.plugin.getLogger().info("Failed to update header/footer");
            DragonNiteMC.plugin.getLogger().info("Try check your configuration");
        }
    }
